...
def DynamicRig():
    ...
    if len(sel)<=2:
        ...
        start_curve_attract=cmds.addAttr(sel[0],ln="StartCurveAttract",at="float",min=0,max=1,dv=1,k=True)
        ...
        cmds.connectAttr(sel[0]+".StartCurveAttract",hairshapenodes[1]+".startCurveAttract")
...
def controlatselection():
    ...
    if len(sel)==0:
        ...
        for i in sel:
            ...
            cmds.select(cl=1)
            jnt = cmds.joint(n=i.replace("_Jnt","_jnt"))
            cmds.matchTransform(jnt,i)
            cmds.select(cl=1)
            
            ctrl=cmds.circle(ch=0,r=3,nr=(1,0,0),n=i.replace("_Jnt","_Ctrl"))
            cmds.setAttr(ctrl[0]+".overrideEnabled",1)
            cmds.setAttr(ctrl[0]+".overrideColor",17)
            
            ctrl_grp=cmds.group(ctrl,n=i.replace("_Jnt","_Ctrl_Group"))
...
def follicelcontrol():
    ...
    if len(sel)==0:
        ...
        for i in sel:
            ...
            cmds.setAttr(circle_ctrl[0]+".overrideEnabled",1)
            cmds.setAttr(circle_ctrl[0]+".overrideColor",22)
